...
namespace Molytho.Matrix.Calculation.Providers
{
    internal class FloatCalculationProvider : ICalculationProvider<float>
    {
        ...
        private unsafe void TransposeAvx2(MatrixBase<float> ret, MatrixBase<float> a)
        {
            fixed (float* base_a = &a[0, 0], base_ret = &ret[0, 0])
            {
                ...
                {
                    0,
                    a.Width * 1 * sizeof(float),
                    a.Width * 2 * sizeof(float),
                    a.Width * 3 * sizeof(float),
                    a.Width * 4 * sizeof(float),
                    a.Width * 5 * sizeof(float),
                    a.Width * 6 * sizeof(float),
                    a.Width * 7 * sizeof(float)
                };
                Vector256<int> indexVec = Avx.LoadVector256(indices);
                if (a.Height >= Vector256<float>.Count)
                {
                    int indexScale = a.Width * 8 * sizeof(float);
                    Vector256<int> indexVecInc = Avx2.BroadcastScalarToVector256(&indexScale);
                    while (calculated + Vector256<float>.Count < a.Height)
                    {
                        if (calculated > 0)
                        ...
                        {
                            Vector256<float> storeVec = Avx2.GatherVector256(base_a + x, indexVec, 1);
                ...
                if (a.Height % Vector256<float>.Count > 1)
                {
                    ...
                    for (int x = 0; x < a.Width; x++)
                    {
                        Vector256<float> storeVec = Avx2.GatherMaskVector256(Vector256<float>.Zero, base_a + x, indexVec, maskVec, 1);
                        Avx2.MaskStore(base_ret + x * ret.Width + calculated, maskVec, storeVec);
                    }
                }
                else if (a.Height % Vector256<float>.Count == 1)
                {
                    for (int x = 0; x < a.Width; x++)
                    {
        ...
        public unsafe MatrixBase<float> Transpose(MatrixBase<float> a)
        {
            MatrixBase<float> ret =
                a.Height == 1
            ...
            {
                fixed (float* base_a = &a[0, 0], base_ret = &ret[0, 0])
                    MemoryOperation.memcpy(base_ret, base_a, a.Height * a.Width * sizeof(float));
            }
